...
export default function AdminInicio() {
  ...
  const cargarDatosLocales = () => {
    ...
    if (usuarioLogueado) {
      ...
      const nombreCompleto = datosUsuario.full_name || datosUsuario.nombre_completo || datosUsuario.username || "Administrativo";
  ...
  const handleSubirImagen = async (e) => {
    ...
    // Puedes usar el mismo preset o crear uno específico para admins en Cloudinary
    const uploadPreset = import.meta.env.VITE_CLOUDINARY_PRESET_DOCENTES; 
  ...
  const handleGuardarPerfil = async (e) => {
    ...
    if (!editForm.correo || editForm.correo.trim() === "") return alert("⚠️ El correo electrónico es obligatorio.");
    if (!editForm.telefono || editForm.telefono.trim() === "") return alert("⚠️ El número de celular es obligatorio.");
    if (!editForm.foto_perfil || editForm.foto_perfil.trim() === "") return alert("⚠️ Es obligatorio seleccionar una foto.");

    const noHuboCambios = editForm.correo === adminData.correo && editForm.telefono === adminData.telefono && editForm.foto_perfil === adminData.foto_perfil;
    if (noHuboCambios) return alert(" No has realizado ninguna modificación.");

    try {
      const respuesta = await modificarPerfilAdmin(adminData.id_usuario, editForm);
  ...
  return (
    <div className="w-[95%] max-w-[1800px] mx-auto pt-6 pb-12 animate-fade-in-up font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center md:items-end mb-10 relative px-4">
        <div className="flex flex-col md:flex-row items-center gap-4 mb-6 md:mb-0">
          ...
          <div className="text-center md:text-left">
            ...
            <h2 className="text-4xl text-[#0033a0] font-bold">{adminData.nombre}</h2>
          </div>
        </div>
      ...
      {/* ========================================== */}
      {/* SECCIÓN PRINCIPAL: UNA SOLA COLUMNA        */}
      {/* ========================================== */}
      <div className="w-full px-4 mt-8">
        {/* Descomentar cuando creemos el componente */}
            ...
            <div className="p-6 overflow-y-auto">
              <form onSubmit={handleGuardarPerfil}>
                  ...
                  <div className="bg-gray-50 p-5 rounded-xl border border-gray-200">
                    ...
                    <div className="flex flex-col items-center mb-6">
                      <div className="w-24 h-24 rounded-full overflow-hidden border-4 border-white shadow-sm bg-gray-200 mb-2">
                        {adminData.foto_perfil ? (
                          <img src={adminData.foto_perfil} alt="Actual" className="w-full h-full object-cover" />
                        ) : (
                      ...
                      <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Foto actual</span>
                    </div>
                    <div className="space-y-4">
                      <div>
                        <label className="block text-xs font-bold text-gray-500 uppercase">Celular / Teléfono</label>
                        <p className="font-semibold text-gray-800 bg-white p-2 rounded border border-gray-200 mt-1">{adminData.telefono || "No registrado"}</p>
                      </div>
                      <div>
                        <label className="block text-xs font-bold text-gray-500 uppercase">Correo Electrónico</label>
                        <p className="font-semibold text-gray-800 bg-white p-2 rounded border border-gray-200 mt-1 truncate">{adminData.correo || "No registrado"}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-2">
                    ...
                    <div className="space-y-5">
                      <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
                        <label className="block text-sm font-bold text-[#0033a0] mb-2">Cambiar Foto</label>
                        <input type="file" accept="image/*" onChange={handleSubirImagen} className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-[#0033a0] file:text-white hover:file:bg-blue-800 transition-all cursor-pointer" />
                        {isUploading && <p className="text-sm text-blue-600 font-bold mt-2 animate-pulse">Subiendo imagen...</p>}
                        {editForm.foto_perfil && editForm.foto_perfil !== adminData.foto_perfil && !isUploading && <p className="text-sm text-green-600 font-bold mt-2">¡Nueva imagen lista!</p>}
                      </div>
                      <div>
                        <label className="block text-sm font-bold text-gray-700 mb-1">Nuevo Celular</label>
                        <input type="tel" className="w-full border border-gray-300 rounded-lg p-2.5 focus:outline-none focus:border-[#0033a0] focus:ring-1 focus:ring-[#0033a0] transition-colors bg-gray-50 focus:bg-white" value={editForm.telefono} onChange={(e) => setEditForm({ ...editForm, telefono: e.target.value })} />
                      </div>
                      <div>
                        <label className="block text-sm font-bold text-gray-700 mb-1">Nuevo Correo</label>
                        <input type="email" className="w-full border border-gray-300 rounded-lg p-2.5 focus:outline-none focus:border-[#0033a0] focus:ring-1 focus:ring-[#0033a0] transition-colors bg-gray-50 focus:bg-white" value={editForm.correo} onChange={(e) => setEditForm({ ...editForm, correo: e.target.value })} />
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex justify-end gap-3 pt-6 mt-6 border-t border-gray-200">
                  <button type="button" onClick={() => setIsEditModalOpen(false)} className="px-5 py-2.5 rounded-lg font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 transition-colors">Cancelar</button>
                  <button type="submit" disabled={botonDeshabilitado} className={`px-5 py-2.5 rounded-lg font-bold text-white shadow-md transition-all ${botonDeshabilitado ? 'bg-gray-300 text-gray-500 cursor-not-allowed shadow-none' : 'bg-[#0033a0] hover:bg-blue-800 cursor-pointer'}`}>
                    {isUploading ? "Subiendo..." : noHuboCambios ? "Sin Cambios" : "Guardar Cambios"}
                  </button>
                </div>
              </form>
            </div>
