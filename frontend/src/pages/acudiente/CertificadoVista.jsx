...
export default function CertificadoVista() {
  ...
  const { estudiante, tipo, notas } = location.state || {};
  ...
  return (
    <div className="w-full min-h-screen bg-gray-50 sm:p-4 lg:p-6 flex flex-col items-center animate-fade-in-up">
      <div className="bg-white border-0 sm:border-2 border-gray-200 rounded-none sm:rounded-xl shadow-none sm:shadow-md p-4 sm:p-6 md:p-8 flex flex-col items-center w-full max-w-[1800px] flex-1 sm:flex-none">
        ...
        {/* Barra de Herramientas Estilo Estudiante */}
        <div className="w-full flex flex-col md:flex-row justify-between items-start md:items-center mb-6 sm:mb-8 bg-blue-50 p-4 sm:p-6 rounded-lg border-l-4 border-[#0033a0] shadow-sm gap-4">
          <div className="flex-1 text-left w-full">
            ...
            <p className="text-xs sm:text-sm text-gray-600 font-medium mt-1">Revisa el documento antes de descargarlo.</p>
          </div>
          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            <button onClick={() => navigate(-1)} className="w-full sm:w-auto bg-gray-200 text-gray-700 px-6 py-3 sm:py-2 rounded-lg font-bold hover:bg-gray-300 transition-all shadow-sm text-center">
              Volver
            </button>
            <button onClick={descargarPDF} className="w-full sm:w-auto bg-[#0033a0] text-white px-6 py-3 sm:py-2 rounded-lg font-bold hover:bg-blue-800 transition-all shadow-md flex items-center justify-center gap-2">
        ...
        {/* CONTENEDOR DEL DOCUMENTO A4 (Fondo gris con sombra para simular la hoja) */}
        <div className="w-full overflow-x-auto bg-gray-100 p-4 sm:p-8 rounded border border-gray-300 flex justify-center custom-scrollbar">
          ...
          {/* HOJA EXACTA DE ANTES */}
          <div ref={certificadoRef} className="bg-white shadow-lg sm:shadow-2xl w-[210mm] min-w-[210mm] min-h-[297mm] p-10 md:p-20 text-gray-800 flex flex-col mx-auto" style={{ fontFamily: "'Times New Roman', Times, serif" }}>
            ...
            <div className="text-justify text-base sm:text-lg leading-relaxed space-y-6 flex-1">
              ...
              {tipo === "Conducta" && (
                <p>Que el(la) estudiante <span className="font-bold">{estudiante.nombre}</span>, durante su proceso formativo en esta institución, ha observado un comportamiento ajustado a las normas de convivencia, demostrando valores de respeto, responsabilidad y compañerismo. Por tal motivo, se le asigna la calificación de <span className="font-bold uppercase">Excelente</span> en conducta.</p>
              )}
              {tipo === "Notas" && (
                  ...
                  <table className="w-full border-collapse border border-gray-400 text-xs sm:text-sm">
                    <thead>
                      <tr className="bg-gray-100 uppercase">
                        ...
                        <th className="border border-gray-400 p-2 text-center w-10 sm:w-16">P1</th>
                        <th className="border border-gray-400 p-2 text-center w-10 sm:w-16">P2</th>
                        <th className="border border-gray-400 p-2 text-center w-10 sm:w-16">P3</th>
                        <th className="border border-gray-400 p-2 text-center w-14 sm:w-20">Final</th>
                      </tr>
                    </thead>
                    <tbody>
                      {notas && notas.length > 0 ? notas.map((n, idx) => (
                        <tr key={idx}>
                          <td className="border border-gray-400 p-2 font-medium break-words">{n.materia}</td>
                      ...
                      )) : <tr><td colSpan="5" className="border border-gray-400 p-4 text-center">No hay calificaciones registradas.</td></tr>}
                    </tbody>
                  </table>
                </div>
              )}
            ...
            <div className="mt-16 sm:mt-20 flex flex-col items-center">
              <div className="w-48 sm:w-64 border-t border-black mb-2"></div><p className="font-bold text-center text-sm sm:text-base">RECTORÍA GENERAL</p><p className="text-xs sm:text-sm text-gray-600">Centro Educativo Salvador Lenis</p>
            </div>
          </div>
