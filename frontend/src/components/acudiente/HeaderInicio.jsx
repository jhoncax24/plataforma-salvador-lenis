...
const Header = ({ user }) => {
  ...
  const styles = {
    header: {
      ...
      boxShadow: '0 4px 6px rgba(0,0,0,0.1)', borderBottom: '4px solid #D32F2F',
  ...
  return (
    <header style={styles.header}>
      <Link to="/acudiente" style={styles.logo}>
        <span style={{ color: '#D32F2F' }}>Portal</span> Acudiente
      </Link>
      <nav style={styles.nav}>
        <Link to="/hijos" style={styles.link}>Mi hijo/a</Link>
        {/* <Link to="/acudiente" style={styles.link}>Mi Perfil</Link> */}
        ...
        <div style={styles.userSection}>
          <div style={{ textAlign: 'right', fontSize: '0.9rem', fontWeight: 'bold' }}>{user?.nombre || 'Acudiente'}</div>
          <FaUserCircle size={25} />
          {/* <button onClick={logout} style={{ background: 'none', border: 'none', color: 'white', cursor: 'pointer' }} title="Cerrar Sesión">
